...
class EGNNLayer(nn.Module):
    def __init__(self, num_hidden):
        super().__init__()
        self.message_mlp = nn.Sequential(nn.Linear( num_hidden, num_hidden), nn.SiLU(), nn.Linear(num_hidden, num_hidden), nn.SiLU())
    ...
    def forward(self, x, pos, edge_index, pe= 0):
        send, rec = edge_index
...
class EGNN(nn.Module):
    def __init__(self, num_in, num_hidden, num_out, num_layers):
        super().__init__()
        self.embed = nn.Linear(num_in+24, num_hidden)
...
if __name__ == '__main__':
    wandb.init(project=f"DL2-EGNN", name="floor_equation_sum_yes_pe_no_lspe")
